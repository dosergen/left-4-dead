...
int TanksSpawned = 0;
int TanksToSpawn = 0;
int TanksMustSpawned = 0;
int TanksFrustrated = 0;
int DefaultMaxZombies = 0;
...
float g_FirstTankPos[3];
float g_BoxA[10][3];
float g_BoxB[10][3];
float g_SpawnPos[10][3];
bool g_SpawnFix;
int g_SpawnFixes;

int propinfoburn = - 1;

Handle CurrentGameMode = INVALID_HANDLE;

Handle MTDebug	= INVALID_HANDLE;
Handle MTOn		= INVALID_HANDLE;

Handle MTCountRegularCoop	= INVALID_HANDLE;
Handle MTHealthRegularCoop	= INVALID_HANDLE;
Handle MTHealthFinaleCoop	= INVALID_HANDLE;
Handle MTCountFinaleCoop	= INVALID_HANDLE;
Handle MTHealthFinaleStartCoop	= INVALID_HANDLE;
Handle MTCountFinaleStartCoop	= INVALID_HANDLE;
Handle MTHealthFinaleStart2Coop	= INVALID_HANDLE;
Handle MTCountFinaleStart2Coop	= INVALID_HANDLE;
Handle MTHealthEscapeStartCoop	= INVALID_HANDLE;
Handle MTCountEscapeStartCoop	= INVALID_HANDLE;

Handle MTHealthRegularVersus	= INVALID_HANDLE;
Handle MTCountRegularVersus	= INVALID_HANDLE;
Handle MTHealthFinaleVersus	= INVALID_HANDLE;
Handle MTCountFinaleVersus	= INVALID_HANDLE;
Handle MTHealthFinaleStartVersus	= INVALID_HANDLE;
Handle MTCountFinaleStartVersus	= INVALID_HANDLE;
Handle MTHealthFinaleStart2Versus	= INVALID_HANDLE;
Handle MTCountFinaleStart2Versus	= INVALID_HANDLE;
Handle MTHealthEscapeStartVersus	= INVALID_HANDLE;
Handle MTCountEscapeStartVersus	= INVALID_HANDLE;

Handle MTHealthSurvival	= INVALID_HANDLE;
Handle MTCountSurvival		= INVALID_HANDLE;

Handle MTHealthScavenge	= INVALID_HANDLE;
Handle MTCountScavenge	= INVALID_HANDLE;

Handle AnnounceTankHP	= INVALID_HANDLE;
Handle MTAutoSpawn	= INVALID_HANDLE;
Handle MTSpawnTogether	= INVALID_HANDLE;
Handle MTSpawnTogetherFinal	= INVALID_HANDLE;
Handle MTSpawnTogetherEscape	= INVALID_HANDLE;
Handle MTSpawnDelay	= INVALID_HANDLE;
Handle MTSpawnCheck	= INVALID_HANDLE;
Handle MTSpawnDelayEscape	= INVALID_HANDLE;

Handle SpawnTimer    		= INVALID_HANDLE;
Handle CheckTimer    		= INVALID_HANDLE;

Handle MTShowHUD	= INVALID_HANDLE;

Handle MTChangeHP	= INVALID_HANDLE; // 24.05.11 sheleu

bool IsTank[MAXPLAYERS+1];
bool IsFrustrated[MAXPLAYERS+1];
int Frustrates[MAXPLAYERS+1];
bool IsRoundStarted;
bool IsRoundEnded;

Handle logfile;

int L4D_ZOMBIECLASS_TANK	= 5;
int L4D2_ZOMBIECLASS_TANK	= 8;

int ZC_TANK;

int infectedClass[MAXPLAYERS+1];
bool resetGhostState[MAXPLAYERS+1];
bool resetIsAlive[MAXPLAYERS+1];
bool resetLifeState[MAXPLAYERS+1];
bool restoreStatus[MAXPLAYERS+1];

float HUD_UPDATE_INTERVAL		= 1.0;
...
public Plugin myinfo = 
{
	...
	author = "Red Alex & Sheleu [Edited by Dosergen]",
...
//	HookEvent("player_team", PlayerTeam);
	...
	CreateConVar("l4d_multitanks_version", PLUGIN_VERSION, "[L4D & L4D2] MultiTanks version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	...
	MTShowHUD = CreateConVar("mt_showhud","1","Show Tanks HUD?", CVAR_FLAGS,true,0.0,true,1.0);
	...
	logfile = OpenFile("/addons/sourcemod/logs/multitanks_log.txt", "w");
}
...
//	int tankid = GetEventInt(event, "tankid");
...
//	SetEntProp(client, Prop_Send, "m_frustration", 0)
	...
	if (GetConVarInt(MTDebug))
	{
		...
		if (IsFakeClient(client))
			PrintDebug("[TANK] TankSpawn [FAKE] [%d,%d]",tankid,client); 
		else
...
//			GetClientModel(i, stringclass, 32);
			if (IsPlayerTank(i))
			{
				...
				if (GetConVarInt(MTDebug))
				{
					...
					if (IsFakeClient(i))
						PrintDebug("[TANK] === [FAKE] [%d] [%d,%d,%d] [%f,%f,%f]",i, IsClientConnected(i), IsClientInGame(i), GetClientTeam(i), Pos[0],Pos[1],Pos[2]); 
					else
...
//			isTank[client] = true;
	...
	if (g_SpawnFix)
	{
		...
		for (int k=0; k < g_SpawnFixes; k++)
		{
			if ((g_TankPos[0] >= g_BoxA[k][0]) && (g_TankPos[0] <= g_BoxB[k][0]) && (g_TankPos[1] >= g_BoxA[k][1]) && (g_TankPos[1] <= g_BoxB[k][1]))
			{
				PrintToServer("Teleport To %f,%f,%f" ,g_SpawnPos[k][0],g_SpawnPos[k][1], g_SpawnPos[k][2]);
...
public void CalculateTanksParamaters()
{
	switch (g_GameMode)
	{
		case 1:
...
public void OnMapStart()
{
	...
	else if (StrEqual(g_MapName, "l4d_vs_farm03_bridge"))
	{
		g_SpawnFix = true;
		g_SpawnFixes = 1;
		g_BoxA[0][0] = 3152.0;
	...
	if (GetConVarInt(MTDebug) >= 1)
	{
		char mBuffer[128];
		Format(mBuffer, sizeof(mBuffer), "Map: %s", g_MapName);
...
public Action PrintDebug(const char[] format, any:...)
{
	char buffer[192];
	VFormat(buffer, sizeof(buffer), format, 2);

	if (GetConVarInt(MTDebug) >= 2)
	{
		PrintToChatAll("[MT] %s", buffer);
	}
...
public void SaveAndInreaseMaxZombies(int number)
{
	if (isSuperVersus())
	{
...
public void UnsetNotifytVar(Handle hndl)
{
...
public void SetNotifytVar(Handle hndl)
{
	int flags = GetConVarFlags(hndl);
	flags |= FCVAR_NOTIFY;
	SetConVarFlags(hndl, flags);
...
public void OnCVGameModeChange(ConVar convar, const char[] oldValue, const char[] newValue)
{
...
public void Event_finale_escape_start(Event event, const char[] name, bool dontBroadcast)
{
	g_MapState = 3;
	CalculateTanksParamaters();
}

public void Event_finale_vehicle_leaving(Event event, const char[] name, bool dontBroadcast)
{
	g_MapState = 4;
	CalculateTanksParamaters();
}
...
public Action Command_RefreshSettings(int client, int args)
{
	CalculateTanksParamaters();
...
public Action Command_MTTest2(int client, int args)
{
	SetEntProp(client, Prop_Send, "m_frustration", 0);
}

public Action Command_MTTest(int client, int args)
{
	int maxclients;
	maxclients = MaxClients;
	bool isTankClient = false;
...
public Action Command_MTSpawnBot(int client, int args)
{

	for (int i=1; i<=MaxClients; i++) //now to 'disable' all human players
	{
...
public Action CheckFrustration(Handle timer, any client)
{
	...
	int frustration = GetEntProp(client, Prop_Send, "m_frustration");
...
public bool IsPlayerTank (int client)
{
	char class[150];
...
public int HUD_Handler(Handle menu, MenuAction action, int param1, int param2) 
{ 
...
void HUD_Draw()
{
	...
	{	
		if (IsClientInGame(i) && IsPlayerTank(i) && IsPlayerAlive(i) && !IsPlayerIncapped(i))
		{
			...
			int frustration = 100-GetEntProp(i, Prop_Send, "m_frustration");
			int health = GetEntData(i, FindDataMapInfo(i, "m_iHealth"));
			char tBuffer[512];
			...
			if (TotalCount == 1)
				Format(sBuffer, sizeof(sBuffer), "%s", tBuffer); 
			else
...
{
	int propincapped = FindSendPropInfo("CTerrorPlayer", "m_isIncapacitated");
	int isincapped = GetEntData(client, propincapped, 1);
