...
public Action HookSound_Callback(int Clients[64], int &NumClients, char StrSample[PLATFORM_MAX_PATH], int &Entity)
{
	...
	Event newEvent = CreateEvent("entity_shoved", true);
...
{
	int entid = GetEventInt(event, "entityid"); //get the events shoved entity id

	char entclass[96];
	GetEntityNetClass(entid, entclass, sizeof(entclass));
	if (!StrEqual(entclass, "Infected")) return; //make sure it IS a zombie.

	Handle data = CreateDataPack(); //a data pack because i need multiple values saved
...
public Action CheckForMovement(Handle timer, Handle data)
{
	...
	int zombieid = ReadPackCell(data); //get the Zombie id
